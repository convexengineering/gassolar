...
To capture the key trade offs like those presented in Figures~\ref{f:bvslatsolar} and ~\ref{f:mtowvsendurance} a geometric programm is built that models for both gas and solar-electric powered, long-endurance aircraft.  
...
where the solar cell density and the battery energy density are assumed to be $\rho_{\text{solar}} = 0.27$ [kg/m$^2$] and $h_{\text{batt}} = 350$ [Whr/kg].
...
From a design standpoint the sensitivity result is useful in that variables with high sensitivities are important to the vehicle design and should be modeled with higher fidelity. 
...
Because of the wing's large moment of inertia, caused by the weight of the wings, the solar-electric aircraft is considered a highly flexible aircraft. 
...
The horizontal tail is sized to satisfy a conservative horizontal tail volume coefficient condition, $V_{\text{h}} = 0.45$.
...
where $m_M$ is the motor mass, $P_{\text{max}} \geq P_{\text{oper}}$ is the maximum operating power, and the power to mass ration is $B_{PM} = 4140.8$ [W/kg]. \\
...
\begin{equation}
    \label{e:rpmtobsfc}
    \left( \frac{BSFC}{BSFC_{\text{min}}} \right)^{35.7041} \geq 2.24902\left( \frac{P_{\text{shaft}}}{P_{\text{max}}}\right)^{2.72257} + 0.00124367\left( \frac{P_{\text{shaft}}}{P_{\text{max}}} \right)^{-13.0099} \\
\end{equation}

where $BSFC_{\text{min}} = 0.3162$ [lb/hp/hr]. 
...
\begin{figure}[H]
 ...
 \caption{\textbf{Comparison of GP-compatible approximation to RCV manufacturing data for the DF70 engine.  Function is representative of engine performance for engines of similar size.  The log-space RMS error is 0.021. }}
	\label{f:powervsweightfit}
	\end{center}
\end{figure}
...
\begin{equation}
    \label{e:lapsefit}
    L_{\text{eng}} = 1 - \frac{0.035}{1000 \text{[ft]}}
\end{equation}
...
Figure~\ref{f:latvsmtowtrade} was recalculated with the additional constraints included from Section V and is presented in Figure~\ref{latvsmtowtrade2}. 
The higher fidelity models reveal a more realistic limit to the size of the solar-electirc powered aircraft and actually improve the performance of the gas powered aircraft, suggesting that some assumptions about the $BSFC$ and drag may have been too pessimistic. 
