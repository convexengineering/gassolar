...
Geometric programming, a form of convex optimization, is chosen as a means of evaluating the design space because of its ability to solve optimization problems involving thousands of variables in seconds.  
...
To achieve a feasible design, solar-electric powered aircraft must carry enough solar cells and batteries required to fly during the day while storing enough energy to fly through the night. 
...
An analysis was done of how wind speeds vary from month to month to determine during which seasons or months are the wind speeds the worst. Figure~\ref{f:windvsmonth} reveals that wind speeds are significantly worse during the winter months. 
...
For this reason, latitude was not included as a variable in the fitted equation.  
Instead to optimize an aircraft for 35 degrees latitude means that the there should exist 70 separate wind and solar flux constraints that must be satisfied, one for each latitude.
...
The constraint of wind speed and air density at the 35 degree latitude is shown in Equation~\eqref{e:windfit35}.
...
Under these assumptions, if the aircraft can fly for one day and one night, then it could theoretically fly for many days under similar circumstances. 
    ...
    $P_{\text{oper}}$ is defined as the average power required to operate the aircraft. \eqref{e:solarpoper} 
...
One way to interpret Figure~\ref{f:latvsmtowtrade} is that a solar-powered aircraft weighing 175 lbs is able to operate between 0 and 40 degrees North latitude in 90th percentile wind speeds.  
...
For a gas-powered architecture, achieving endurances above 10 days is likely impossible to build.  Solar-electric powered aircraft, however, can theoretically fly indefinitely. 
...
where the taper ratio is $\lambda=0.5$.  Thus, the pre-computed distributed load 
...
Because it is assumed that the batteries are located inside the wings for the solar aircraft, the key sizing case for the solar-electric aircraft spar is a gust load that varies across the span.  Assuming the lifting distribution can be split into an elliptical lifting distribution and a gust lifting distribution the loading distribution can be expressed as
...
such that the weight of the wing is assumed to vary linearly with the chord. 
By approximating the lift as $L=W=W_{\text{wing}} + W_{\text{cent}}$, the non-dimensionalize distribubted gust load $\bar{q}$, can be expressed as
...
The horizontal tail is sized to meet a minimum static margin $\text{SM}_{\text{min}} = 0.35$ and minimum desirable c.g. travel range,
...
For the gas powered aircraft it is assumed that the fuel is carried in an elliptically shaped fuselage.  The fuselage will increase the overall weight and drag of the aircraft.  The solar-electric powered aircraft is assumed to carry the batteries in the wings will therefore have no fuselage.  
...
where $k_{fuse}$ is the form factor approximated by 
...
This engine performance mapping can show how aircraft parameters vary during the mission. 
...
The lapse rate $L_{\text{eng}}$, is assumed to affect the maximum power output,
...
The lapse is calculated from the required flight altitude $h=15,000$ [ft], prior to the optimization solve using a linear regression fit from the manufacture data for the DF70 engine from RCV Engines
...
To ensure that the aircraft climbs at a reasonable climb rate a climb rate constraint is enforced in the optimization model, 
...
A comparison of the solar-electric powered latitude plots with and without structural models reveals a more realistic limit to the wing span and aircraft size and a larger effect of latitude on aircraft size. 
