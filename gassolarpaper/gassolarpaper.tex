...
An empennage model with a single tail constisting of a tail boom, horizontal and vertical tail, was added to both the solar-electric and gas powered aircraft.  
...
where the index $k=0$ corresponds to a uniform wall thickness and stiffness, and $k=1$ corresponds to a linear dropoff to zero.  For both the solar-electric and gas powered aircraft, $k=0.8$.  When the tail boom is loaded at the endpoint, $x=l_h$, by the horizontal tail lift, $L_h$, th end deflection angle follows from standard beam analysis. 
...
The horiztonal tail is sized to meet a minimum static margin, $SM_{min} = 0.35$ and minimum desirable c.g. travel range,

\begin{equation}
    \label{e:deltacg}
    \Delta_{cg} = (x_{cg})_{aft} - (x_{cg})_{fwd} = 0.2
\end{equation}
...
The vertical tail is sized to meet a conservative volume tail coefficient, $V_v= 0.05$,
...
\begin{align}
    ...
    \label{e:taildrag}
    Cd^{70.5599} &\geq 7.42688e-90(Re/1e^3)^{-33.0637}
         (100 \tau )^{18.0419}
        + 5.02826e-163(Re/1e^3)^{-18.7959}
         (100\tau)^{53.1879}
        + 4.22901e-77(Re/1e^3)^{-41.1704}
         (100\tau)^{28.4609}
\end{align}

where $\tau = 0.8$ is the airfoil thickness. Assuming steady level flight where neither surface is generating lift, the XFOIL data was generated a zero angle of attack. A comparison of the XFOIL data and Equation~\ref{e:taildrag} is shown in Figure~\ref{f:taildragpolar}

\begin{figure}[H]
	\begin{center}
	\includegraphics[width=0.6\textwidth]{taildragpolar.pdf}
    \caption{ \textbf{ Comparison of XFOIL data for NACA airfoils to GP-compatible generated fitComparison of XFOIL data for NACA airfoils to GP-compatible generated fit.~\eqref{e:taildrag} RMS error is 0.0173.}}
...
where the fuel is assumed to be Jet A fuel, $\rho_{fuel} = 6.01$ [lbf/gallon].The volume of the fuselage is constrained by
...
where $l$ is the length of the fuselage and $R$ is the radius. Using the length and radius the surface area can be calculated using an approxmiation,\cite{ellipsoidSA}
...
where $k_{fuse}$ is the form factor approxmiated as
...
The equations governing fuselage weight and drag were added to the overall gas powered optimization model.
...
where $m_M$ is the motor mass, the maximum power, $P_{max}$, is determined by the operating power, and the power to mass ration is, $B_{PM} = 4140.8$ [W/kg].
...
where $W_{engine-ref} = 10$ [lbs] and $P_{ref} = 10$ [hp].  Equation~\ref{e:powerlaw} was found using techniques described in Hoburg\cite{fitting}. Equation~\ref{e:powerlaw} is compared to the data in Figure~\ref{f:powervsweightfit}
...
To account for these two affects a two-stroke, single cylinder engine, the DF70 from RCV Engines, was selected and its manufacturing data used to generate representative performance curves.  
...
\begin{align}
    \label{e:rpmtobsfc}
    \left( \frac{BSFC}{BSFC} \right)^{36.2209} &\geq 2.31541\left( \frac{RPM}{RPM_{max}}\right)^{8.06517} + 0.00103364\left( \frac{RPM}{RPM_{max}} \right)^{-38.8545} \\
    \label{e:rpmtopower}
...
\begin{figure}[H]
 ...
 \end{subfigmatrix}
 \caption{\textbf{Comparison of GP-compatible approximations to RCV manufacturing data for the DF70 engine.  Functions used to represent engine performance for engines of similar size.  RMS error for $BSFC$ function is 0.021.  RMS error for power function is 0.0093. }}
 \label{f:RCVdata}
\end{figure}

The lapse rate, $L_{eng}$, is assumed to affect the power out,
...
where $P_{SL-max}$ is used to size the engine in Equation~\ref{e:powerlaw} and $P_{max}$ determines the engine performance.~\eqref{e:rpmtopower}

The laspse is calculated from the flight requirement altitude prior to the optimization solve using a linear regression fit from the manufacture data for the DF70 engine from RCV Engines

\begin{equation}
    \label{e:lapsefit}
    L_{eng} = -3.1802e^{-5} h + 0.9730
\end{equation}

where the altitude, $h = 15,000$ [ft]. A comparison of the manufacture data and linear regression fit for the lapse rate is shown in Figure~\ref{f:lapsefit}

\begin{figure}[H]
	\begin{center}
	\includegraphics[width=0.6\textwidth]{lapseline.pdf}
    \caption{ \textbf{ Comparison of manufacture data to linear regression fit~(\eqref{e:lapsefit}) for the DF70 engine from RCV Engines.}}
	\label{f:lapsefit}
	\end{center}
\end{figure}


\subsection{Climb Model}
...
Additional models also improve the optimization accuracy by increasng variable interaction. 
...
However, by adding a structural model larger wings now have a weight penalty and according to Equation~\ref{e:slfweight}, for the same $C_L$, if you have a smaller wing higher air density will generate more lift. 
...
Interesting, a direct comparison between the simple and detailed optimization models shows a cross over around an endurance of 6 days. (Figure~\ref{f:simpdetcomp}) The comparison was run at a latitude of 38 degress and 90th percentile winds.
...
An analysis of the non-wing drag coefficenct (Figure~\ref{f:nonwingdrag}) reveals that cross over between the static non-wing drag coefficient, $c_{d_0}=0.01$ and the more detailed component drag breakdown is one of the main causes of the discrepency between the detailed and non-detailed optimization models. 
The decrease in non-wing drag, even as the size of the aircraft increases is due to a Reynolds number effect.  
For a constant airspeed, air density and viscosity, as the aircraft increases in size Reynolds number will increase which will decrease the drag profile coefficient. 

Additionally, a more detailed performance model can show how aircraft parameters vary during the mision. 
...
Table~\ref{t:windvals} lists the values of the coefficients and exponents of Equation~\ref{windspeedgen} for latitudes 20-60. 
...
To derivation of Equation~\ref{e:smcorr} begins with the moment about the aircraft's center of mass
...
\begin{equation}
    \frac{d\epsilon}{d\alph} = \frac{m_W}{4\pi} \frac{c}{l_h}
\end{equation}

so that \eqref{e:eq10} can be wrtten as

\begin{equation}
    C_{L_h} &= m_h \left[\left( 1 - \frac{d\epsilon}{d\alpha}\right) - \frac{qS_hl_h^2}{EI_0}(1-\frac{1}{2}k) C_{L_h}\right] + C_{L_{h_{\delta}}}(\delta_e - \delata_{e_0}) 
\end{equation}

This can be further simplified by defining a tailboom flexibility factor $\mathcal{F}$.

\begin{align}
    C_{L_h} &= \mathcal{F}^{-1} m_h \left( 1 - \frac{d\epsilon}{d\alpha}\right) \alpha + \mathcal{F}^{-1} C_{L_{h_{\delta}}}(\delta_e - \delata_{e_0}) \\
...
\begin{align}
    C_m &= C_{m_W} + \frac{x_{cg} - x_{ac}}{c} (C_{L_{W_0}} + m_W \alpha) - V_h \left[ \mathcal{F}^{-1} m_h \left( 1 - \frac{d\epsilon}{d\alpha}\right) \alpha + \mathcal{F}^{-1} C_{L_{h_{\delta}}}(\delta_e - \delata_{e_0})\right]
...
Dividing Equation~\ref{e:eq6} by $C_{L_W}$, gives a requirement on the tail lift coefficient required to achieve a pitch trim condition $C_m=1$
